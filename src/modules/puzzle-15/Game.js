...
const _direction = {
  ...
  left: {
    offset: 1,
    check(i) {
      return i < 16 && i !== 12 && i !== 8 && i !== 4;
    },
  },
  right: {
    offset: -1,
    check(i) {
      return i >= 0 && i !== 11 && i !== 7 && i !== 3;
    },
  },
};
  ...
  [_move](direction) {
    ...
    if (
      ...
      throw new RangeError(
        "Недопустимый аргумент! Метод принимает только: up, down, left, right."
      );
  }
  ...
  [_update]() {
    ...
    if (this[_isCompleted]) {
      ...
      try {
        ...
        if (recorded) {
          now.time =
            Number(now.time.split(" ")[0]) < Number(recorded.time.split(" ")[0])
              ? now.time
              : recorded.time;
          now.steps =
            Number(now.steps) < Number(recorded.steps)
              ? now.steps
              : recorded.steps;
        }
        console.log(now);
